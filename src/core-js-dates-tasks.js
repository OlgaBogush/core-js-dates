...
function getNextFriday(date) {
  ...
  const array = [5, 4, 3, 2, 1, 7, 6];
  return new Date(obj.setDate(obj.getDate() + array[dayOfWeek]));
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (new Date(dateEnd) - new Date(dateStart)) / (3600 * 24 * 1000) + 1;
}
...
function getCountWeekendsInMonth(month, year) {
  const obj = new Date(year, month, 0);
  const days = obj.getDate();
  let counter = 0;
  for (let i = 1; i <= days; i += 1) {
    const newObj = new Date(year, month - 1, i);
    if (newObj.getDay() === 0 || newObj.getDay() === 6) {
      counter += 1;
    }
  }
  return counter;
...
function getNextFridayThe13th(date) {
  ...
  for (let i = 0; i <= 11; i += 1) {
    const day = new Date(obj.getFullYear(), i, 13);
    if (day.getDay() === 5) {
      a = day;
      break;
    }
...
function getQuarter(date) {
  const obj = new Date(date);
  return Math.ceil((obj.getMonth() + 1) / 3);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  for (let i = 0; i < diff; i += step) {
    for (let j = 0; j < countWorkDays; j += 1) {
      const currentDate = new Date(start.setDate(start.getDate() + j + i));
      ...
      start.setDate(start.getDate() - j - i);
    }
  }
  return array;
}
